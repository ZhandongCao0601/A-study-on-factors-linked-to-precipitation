---
title: "A study on factors linked to precipitation^[Code and data are available in this GitHub repository: https://github.com/ZhandongCao0601/A-study-on-factors-linked-to-precipitation.git]"
date: "26 April 2022"
author: "Zhandong Cao"
lang: "en"
output:
...
library(ggplot2)
ggplot(precipitation, aes(x=mslp))+
  geom_histogram(color="darkblue", fill="lightblue")+
  theme_minimal()+
  labs(title = "Fig 0.1: Distributions of the variables (barplot of mslp)")
```
We can see that the majority of the data was Distributed from -250 to 500 mslp. 

```{r echo=FALSE, fig.height=4, message=FALSE, warning=FALSE}
library(ggplot2)
ggplot(precipitation, aes(x=sstatlan))+
  geom_histogram(color="darkblue", fill="lightblue")+
  theme_minimal()+
  labs(title = "Fig 0.2: Distributions of the variables (barplot of Sea-surface temperature )")
```
...
The other two key methods that I will use to anaylise the data is by examing P-value and performing a t-test on the statistical model that was build. 
...
1. *For what purpose was the dataset created? Was there a specific task in mind? Was there a specific gap that needed to be filled? Please provide a description.*
    - The dataset was innitialy created to serve as agricultural use. It is to record tha precipitation of area across the UK.
2. *Who created the dataset (for example, which team, research group) and on behalf of which entity (for example, company, institution, organization)?*
    - This dataset was created by a group of scientist from the 90s, they are:Alexander and Jones (2001) Jones and Conway (1997) Gregory J. M. and Wigley(1991) T. M. L. Wigley and Jones (1987) J. M. L. Wigley T. M. L. and Jones (1984)
...
1. *What do the instances that comprise the dataset represent (for example, documents, photos, people, countries)? Are there multiple types of instances (for example, movies, users, and ratings; people and interactions between them; nodes and edges)? Please provide a description.*
	- This data set was recorded in the UK only, and it served as national data.
	and is sepreated by different measurement that the scientiset need. They are sea-level data, water vapour data, and wind data.
